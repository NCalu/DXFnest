...
namespace ACadSharp.IO.DXF
{
	internal class DxfObjectsSectionReader : DxfSectionReaderBase
	{
        ...
        private CadTemplate readSortentsTable()
        {
            ...
            ulong? sortHandle = null;
            ulong? entityHandle = null;
            ...
            while (this._reader.DxfCode != DxfCode.Start)
            {
                ...
                {
                    case 5:
                        sortHandle = this._reader.ValueAsHandle;
                        break;
                    case 330:
                        template.BlockOwnerHandle = this._reader.ValueAsHandle;
                        break;
                    case 331:
                        entityHandle = this._reader.ValueAsHandle;
                        break;
                    default:
                        this._builder.Notify(
                            $"Group Code not handled {this._reader.GroupCodeValue} for {typeof(SortEntitiesTable)}, code : {this._reader.Code} | value : {this._reader.ValueAsString}"
                        );
                        break;
                }

                if (sortHandle.HasValue && entityHandle.HasValue)
                {
                    template.Values.Add(new KeyValuePair<ulong?, ulong?>(sortHandle.Value, entityHandle.Value));
                    sortHandle = null;
                    entityHandle = null;
                }
