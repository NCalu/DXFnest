...
namespace ACadSharp.IO.DWG
{
	internal partial class DwgObjectWriter : DwgSectionIO
	{
		private void writeEntity(Entity entity)
		{
            ...
            //Ignored Entities
            if (entity is TableEntity || entity is UnknownEntity || entity is Solid3D || entity is Mesh)
            {
                this.notify(
                    $"Entity type not implemented {entity.GetType().FullName}",
			...
			switch (entity)
			{
				...
				case Tolerance tolerance:
					...
					break;
				case Vertex vertex:
                    if (vertex is Vertex2D vertex2D)
                    {
                        this.writeVertex2D(vertex2D);
                    }
                    else if (vertex is VertexFaceRecord faceRecord)
                    {
                        this.writeFaceRecord(faceRecord);
                    }
                    else if (vertex is Vertex3D || vertex is VertexFaceMesh)
                    {
                        this.writeVertex(vertex);
                    }
                    else
                    {
                        throw new NotImplementedException($"Vertex not implemented : {vertex.GetType().FullName}");
                    }
                    break;
