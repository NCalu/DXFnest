	...
	internal partial class DwgObjectReader : DwgSectionIO
	{
		private CadTemplate readTableEntity()
		{
			...
			if (this.R2010Plus)
			{
				...
				if (hasBreakData)
				{
					...
					for (int i = 0; i < num; i++)
					{
						BreakHeight breakHeight = new BreakHeight();
						//3BD Position
		...
		private void readTableContent(TableContent content, CadTableEntityTemplate template)
		{
			...
			for (int i = 0; i < cols; i++)
			{
				...
				{
					CustomDataEntry entry = new CustomDataEntry();
				...
				CadCellStyleTemplate colStyleTemplate = new CadCellStyleTemplate(column.StyleOverride);
			...
			for (int i = 0; i < nrows; i++)
			{
				...
				//Cell style data, see paragraph 20.4.101.4, this contains cell style overrides for the column.
				CadCellStyleTemplate colStyleTemplate = new CadCellStyleTemplate(row.CellStyleOverride);
				this.readCellStyle(colStyleTemplate);
			...
			{
				CellRange dxfTableCellRange = new CellRange();
				//BL 91 Top row index
		...
		private void readTableCell(CadTableCellTemplate template)
		{
			...
			CadCellStyleTemplate formatTemplate = new CadCellStyleTemplate(cell.StyleOverride);
