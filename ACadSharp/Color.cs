...
namespace ACadSharp
{
	...
	public readonly struct Color : IEquatable<Color>
	{
        ...
        public static byte[] GetIndexRGB(byte index)
        {
            ...
            byte[] rgb = new byte[_indexRgb[index].Length];
            Array.Copy(_indexRgb[index], rgb, _indexRgb[index].Length);
            return rgb;
        }
        ...
        public byte[] GetTrueColorRgb()
        {
            ...
            return new byte[0]; // empty array instead of default
        }
        ...
        {
            byte[] bytes = LittleEndianConverter.Instance.GetBytes(color);
            byte[] rgb = new byte[3];
            Array.Copy(bytes, 0, rgb, 0, 3);
            return rgb;
        }
    }
}