...
namespace ACadSharp.Tables
{
	...
	public class BlockRecord : TableEntry, IGeometricEntity
	{
        ...
        public IEnumerable<Entity> GetSortedEntities()
        {
            ...
            // Use a list of KeyValuePair instead of tuple
            List<KeyValuePair<ulong, Entity>> entities = new List<KeyValuePair<ulong, Entity>>();

            foreach (var entity in this.Entities)
            {
                ulong sorter = this.SortEntitiesTable.GetSorterHandle(entity);
                entities.Add(new KeyValuePair<ulong, Entity>(sorter, entity));
            }

            return entities.OrderBy(e => e.Key).Select(e => e.Value);
        }
